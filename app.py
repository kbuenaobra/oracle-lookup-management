                        ...
                        if conn:
                            try:
                                ...
                                for index, row in df.iterrows():
                                    lookup_type = str(row.get("LOOKUP_TYPE", "")).strip().upper()
                                    lookup_code = str(row.get("LOOKUP_CODE", "")).strip().upper()
                                ...
                                lookup_types_to_insert = [
                                    ...
                                    for lookup_type in sorted({row[0] for row in data_to_insert})
                                ]
                                ...
                                cursor.executemany("""
                                    ...
                                    (LOOKUP_TYPE, LOOKUP_CODE, MEANING, DESCRIPTION, ENABLED_FLAG, 
                                     START_DATE_ACTIVE, END_DATE_ACTIVE, CREATED_BY, CREATION_DATE, LAST_UPDATED_BY, LAST_UPDATE_DATE)
                                    VALUES (:1, :2, :3, :4, :5, :6, :7, 'SYSTEM', SYSDATE, 'SYSTEM', SYSDATE)
                                """, data_to_insert)
